...
def test_clinical_history_agent():
    ...
    try:
        ...
        print(f"  Conditions Found: {len(history_result.conditions)}")
        for cond in history_result.conditions:
            print(f"    - {cond.name} (SNOMED: {cond.snomed_code if hasattr(cond, 'snomed_code') else 'N/A'})")
            
        print(f"\n  Medications Found: {len(history_result.medications)}")
        for med in history_result.medications:
            print(f"    - {med.name} {med.dosage if hasattr(med, 'dosage') else ''}")
